...
export function parseMultipleNights(
          ...
          if (Array.isArray(s.ral)) {
            for (const rally of s.ral) {
              ...
              shots.forEach((sh, i) => {
                ...
                if (sh.sht === 4) { v.atk++; if ((rally.wt === 0 || rally.wt === 1) && rally.wt === pdn[sh.pid]?.team) v.atkW++; }
                if (sh.err?.pop) v.pop++;
                if (sh.err?.f) { v.errTot++; if (sh.err.f.n) v.errNet++; if (sh.err.f.out) v.errOut++; if (sh.err.uf) v.errUf++; }
                if (sh.sht === 1) { v.dinkN++; v.dinkEx += sh.q?.ex ?? 0; }
                if (i >= 2 && sh.sht === 0 && (sh.t?.length ?? 0) >= 2) { const spd = driveSpeedMph(sh.t[0], sh.t[1]); if (spd !== null) { v.dvSum += spd; v.dvN++; } }
                const nx = shots[i + 1];
              ...
              if (t3 && f3) {
                ...
                if (t3.sht === 2) { v.dropN++; if (won) v.dropW++; if (reachedKitchen(shots, pdn, st2, shots.indexOf(t3))) v.dropK++; }
                  ...
                  v.driveN++; if (reachedKitchen(shots, pdn, st2, shots.indexOf(t3))) v.driveK++;
                  const t5 = team[2];
  ...
  const nightTrends: NightTrendRow[] = [...ntMap.values()]
    ...
    .map((v) => ({
      ...
      sqEx: v.sqEx, sqPoor: v.sqPoor, sqW: v.sqW, sdDeep: v.sdDeep, sdW: v.sdW, rdDeep: v.rdDeep, rdW: v.rdW,
